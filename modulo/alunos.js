/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * Objetivo: Obter diferentes dados dos cursos e alunos
 * Data: 15/09/2022
...
//Função para pegar o nome e a foto dos alunos de respectivo curso
const getAlunos = function(curso){
    let nomeCurso = curso.toLowerCase();
    let listAlunos = [];
    let erro = true;

    //Tratamento de erros
    if (nomeCurso != '') {
        //Percorre o array alunos
        alunos.forEach(item => {
            //Percorre o array curso
            item.curso.forEach(curso => {
                //Se o nome do curso identificado for o seguinte, entre na chave
                if (curso.sigla == nomeCurso.toUpperCase()) {
                    //Empurra um JSON com a foto e o nome do aluno dentro
                    listAlunos.push({
                        //Cria um chave no JSON para cada informação
                        foto : item.foto,
                        nome : item.nome,
    ...
    if (erro) {
        return false
    } else { //Identifica qual curso foi selecionado e envia o JSON correspondente com os alunos frequentando o curso
        return listAlunos
    }
}

// Comando para teste da função para listagem dos alunos
// console.log(getAlunos('desenvolvimento'))



//Função para pegar os dados de cada disciplina
const getMatricula = function(matricula){
    ...
    alunos.forEach(item => {
        if (item.matricula == aluno && aluno.length == 11) {
            dadosAluno.push(item.nome)
            item.curso.forEach(curso => {            
                    curso.disciplinas.forEach(disciplinas => {
                        dadosAluno.push(disciplinas)
                    })
                    erro = false  
            })
        } 
...
// console.log(getDisciplinas('Debora'))
